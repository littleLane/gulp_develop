...
Page.prototype.view = function(){
    ...
    if(conf.curr > 1 && conf.prev){
        ...
        view.push('<a href="javascript:;" class="laypage_prev '+ conf.noPrev +'" data-page="'+ (conf.curr - 1) +'">'+ conf.prev +'</a>'); 
    }
    ...
    if(conf.curr !== conf.pages && conf.next || dict.flow){
        ...
        view.push('<a href="javascript:;" class="laypage_prev '+ conf.noNext +'" data-page="'+ conf.curr +'">'+ conf.next +'</a>');
    }
    ...
    return '<div name="laypage'+ laypage.v +'" class="laypage_main laypageskin_'+ (conf.skin ? (function(skin){
        ...
        return conf.skip 
        ? '<span class="laypage_total"><label>&#x8df3;&#x81f3;</label><input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>'
        + '<button type="button" class="laypage_btn" style="font-size: 12px;">&#x786e;&#x5b9a;</button></span>' 
        : '';
    }() +'</div>';
};
