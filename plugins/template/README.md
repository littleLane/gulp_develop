...
> 编译模板并根据所给的数据立即渲染出结果.
	...
	lib.template(tpl, data);

> 仅编译模版暂不渲染，它会返回一个可重用的编译后的函数.

	var compiled_tpl = lib.template(tpl);
...
> 注册/注销自定义函数（对象），在下边 ${变量} 中会有实例.
	...
	lib.template.register('function_name', function);
	lib.template.unregister('function_name');
...
#### 逐条参数更改：
	...
	lib.template.set('strip',false);
	lib.template.set('cache',false);
...
lib.template.register('links_build', links); //注册自定义函数
lib.template(tpl, json);
</code></pre>
...
出于安全角度的考虑，`${变量}` 在输出之前会对其内容进行转义，如果你不想输出结果被转义，可以使用 `$${变量}` 来避免这种情况。例如：
	...
	lib.template(escape_tpl, json); //输出 '&lt;strong&gt;juicer&lt;/strong&gt;'
	lib.template(unescape_tpl, json); //输出 '<strong>juicer</strong>'
...
### 一个完整的例子
	...
	var html = lib.template(tpl, data);
